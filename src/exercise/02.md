...
- This is really really important. To write React applications for the web you
  need two js files.
- React, which is responsible for creating react elements (similar to
  document.createElement()).
- ReactDOM, which is responsible for rendering React elements to the DOM
  (similar to rootElement.append()).
- Code from the React library can run anywhere that you can run js. If you
  wanted you could run it in node.js (though that would be a bit dumb..). You
  then use a platform specific rendering library to render the created react
  elements on whichever platform you're on. On web its ReactDOM. For mobile
  ReactNative etc...
- So, the role of React is to create react elements. The role of ReactDOM is to
  take said elements and put them on the page.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=keaghan.kennedy%40gmail.com).
